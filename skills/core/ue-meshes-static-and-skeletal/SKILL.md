---
name: ue-meshes-static-and-skeletal
...
prefer a soft reference (`TSoftObjectPtr`) loaded on demand — see `ue-asset-management`.
...
`SetMaterial` take precedence. See `ue-materials-and-shaders` for `UMaterialInterface`
...
for `UBodySetup` setup, adding convex shapes, and the `ue-physics-and-chaos` cross-ref.
...
fidelity). See `ue-nanite-and-rendering` for the full rendering pipeline.
...
ecosystem, leader-pose setup, bone queries, and `ue-animation-system` cross-reference.
...
Related skills: `ue-animation-system`, `ue-materials-and-shaders`, `ue-nanite-and-rendering`,
`ue-physics-and-chaos`, `ue-actors-and-components`, `ue-asset-management`.
