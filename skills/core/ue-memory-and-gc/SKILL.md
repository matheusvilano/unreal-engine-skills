...
See also: `ue-cpp-fundamentals` for UObject/UPROPERTY/UFUNCTION macro basics; `ue-actors-and-components`
...
- A `TSoftObjectPtr` holds only a path; call `TSoftObjectPtr::LoadSynchronous()` or use
  `FStreamableManager` to actually load. See `ue-asset-management` for async loading.
