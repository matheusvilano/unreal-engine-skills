...
(`FBodyInstance::bNotifyRigidBodyCollision`). See `ue-actors-and-components` for delegate binding
patterns and `ue-delegates-and-events` for the delegate system.
...
Related skills: `ue-actors-and-components` (component types, overlap wiring),
`ue-meshes-static-and-skeletal` (collision geometry on assets).
