---
name: ue-blueprint-fundamentals
...
- **Cast nodes create hard references** — `Cast To BP_Foo` loads `BP_Foo` into memory when the
  ...
  `ue-asset-management` for soft-reference patterns.
...
- `TObjectPtr<T>` is the modern idiom (UE5+) for member `UPROPERTY` pointers; raw `T*` still
  works but loses access tracking. See `ue-memory-and-gc`.
...
Related skills: `ue-blueprint-cpp-integration`, `ue-actors-and-components`, `ue-cpp-fundamentals`,
`ue-asset-management`.
