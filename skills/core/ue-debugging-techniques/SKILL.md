...
| Question | Tool |
|---|---|
| What value / did this run? | `UE_LOG` / `ensure` — see `ue-logging-and-assertions` |
...
| Frame time, draw calls, physics cost? | `ue-profiling-and-optimization` |
...
`check`, `ensure`, `checkf`, `ensureMsgf` — is in `ue-logging-and-assertions`.
...
Perf regression (fps/ms)        → ue-profiling-and-optimization
```
...
- `Runtime/Core/Public/Misc/AssertionMacros.h` — `check`:229, `checkf`:259,
  `ensure` block:368 (see also `ue-logging-and-assertions`).
...
Related skills: `ue-logging-and-assertions`, `ue-ai-and-navigation`, `ue-profiling-and-optimization`.
