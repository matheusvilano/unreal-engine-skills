...
description: Structure Unreal C++ into modules and configure the build with *.Build.cs
  ...
  discovers and compiles modules. Related to plugins: see ue-plugins-and-modules for packaging
...
- Find which engine module owns a class by locating its header; see `ue-navigating-engine-source`.
...
- `ue-plugins-and-modules` — packaging modules inside a `.uplugin`; plugin vs project module.
- `ue-project-structure` — `.uproject` layout, Source/ and Config/ conventions.
- `ue-navigating-engine-source` — finding which module owns a class or header.
- `ue-cpp-fundamentals` — `UCLASS`/`USTRUCT`/`UENUM`, the reflection system.
