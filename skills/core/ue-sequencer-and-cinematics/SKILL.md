...
beats — the timeline analog of anim notifies (see `ue-animation-system`). For gameplay → timeline
...
- Networked cinematics: trigger the sequence on every client via an RPC/replicated event.
  ...
  See `ue-networking-and-replication`.
...
- For Control Rig-authored animation played inside Sequencer, see `ue-control-rig-and-ik`.
- For skeletal animation tracks and Anim Notifies, see `ue-animation-system`.
