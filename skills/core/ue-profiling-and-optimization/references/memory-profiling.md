...
|---|---|---|
| UObject tag growing | Object leak, no GC trigger | Check for UPROPERTY cycles or raw pointers keeping objects alive; see `ue-memory-and-gc` |
| RenderTargets high | Too many or oversized render targets | Audit render target pool; use `RenderTargetPool` stat |
| Shaders high | Many material permutations | Reduce `StaticSwitch` usage, share base materials; see `ue-materials-and-shaders` |
| Meshes high | Many unique static mesh assets loaded | Cull low-LOD meshes; use Nanite; see `ue-nanite-and-rendering` |
| Textures high | Too many streaming textures resident | Tune streaming pool size; use `stat streaming`; see `ue-asset-management` |
| Memory spike on level load | Hard references pulling large assets | Convert to soft references and async load; see `ue-asset-management` |
