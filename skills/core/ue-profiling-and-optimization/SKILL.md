...
| Symptom | Lever |
|---|---|
| Game thread high, many ticks | Disable unnecessary ticking; use timers/events (`ue-timers-and-async`) |
| Blueprint hot path | Move heavy per-frame logic to C++ |
| Draw thread high | Reduce draw calls: ISM/HISM instancing, fewer unique materials, merge meshes (`ue-materials-and-shaders`, `ue-nanite-and-rendering`) |
| GPU overdraw / fill rate | Reduce translucency layers, cull small objects, lower screen percentage |
| GC spike | Reduce UObject churn; pool objects; tune GC settings (`ue-memory-and-gc`) |
| Memory growth | Avoid hard references pulling large assets; stream assets async (`ue-asset-management`) |
...
Related skills: `ue-nanite-and-rendering`, `ue-materials-and-shaders`, `ue-timers-and-async`,
`ue-memory-and-gc`, `ue-debugging-techniques`.
