...
| --- | --- | --- |
| Time of Day | Primary time control; positions sun/moon by default | See `ue-uds-time` |
...
The Basic Controls category also has an advanced dropdown that holds the **configuration override** option — see `ue-uds-modifiers-configs-state`.
...
- **Space scene** → Sky Mode = Space. See `ue-uds-sun-moon-stars` for stars/space layer.
...
- **Scene renders black after adding UDS** — exposure. Disable any post process volume that overrides exposure; in the viewport's **Lit** dropdown check **Game Settings** for exposure; or on UDS disable **Apply Exposure Settings** in the Exposure category. (See `ue-uds-lighting-and-shadows`.)
...
- **Cloud Coverage / Fog ignored on UDS** — UDW is in the scene. Set them on UDW's weather state instead (see `ue-udw-setup-and-state`).
...
Related skills: `ue-uds-clouds`, `ue-uds-fog-and-atmosphere`, `ue-uds-time`, `ue-uds-lighting-and-shadows`, `ue-uds-performance-mobile-troubleshooting`, `ue-uds-modifiers-configs-state`.
