...
| Volumetric fog below water (extinction + albedo) | Part of Global Volumetric Material (see `ue-uds-fog-and-atmosphere`) |
...
For a fuller soundscape with sound occlusion, prefer UDW's Environment Sound system — see `ue-udw-particles-lightning-wind-sounds`.
...
Some classes have empty functions specifically meant for override in children — e.g. `Custom Volume Behavior` on Weather Override Volume (see `ue-udw-spatial-weather`).
...
- **Weather sounds quiet at wrong times** — sound occlusion using Player Occlusion. Adjust trace behavior in **Player Occlusion** on UDS. (See `ue-udw-particles-lightning-wind-sounds`.)
...
Related skills: `ue-uds-setup-and-modes` (config override option lives in Basic Controls advanced dropdown), `ue-uds-fog-and-atmosphere` (Water Level drives Global Volumetric Material underwater extinction), `ue-uds-performance-mobile-troubleshooting` (`Hard Reset Cache`, `Static Properties` functions), `ue-udw-setup-and-state` (`Apply Weather Configuration`), `ue-udw-particles-lightning-wind-sounds` (Sound Occlusion uses Player Occlusion; Environment Sound system).
