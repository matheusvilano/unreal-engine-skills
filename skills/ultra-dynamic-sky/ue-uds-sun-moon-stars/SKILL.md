...
UDS owns five distinct visual layers above the cloud layer — sun, moon, stars, aurora, and the Space Layer (planets/moons/nebula) — plus a global sky-glow contribution at night. For sun/moon as *directional lights*, see `ue-uds-lighting-and-shadows`. For real-world astronomical positioning, see `ue-uds-simulation`.
...
All ignored when **simulation** is on (see `ue-uds-simulation`).
...
For the filmic Sun Lens Flare effect (separate from engine post-process lens flare), see `ue-uds-modifiers-configs-state`.
...
Most positioning/appearance properties are **static properties** — runtime changes don't apply until you call `Static Properties - Sun`, `Static Properties - Moon`, etc. For an instant full refresh, `Hard Reset Cache`. See `ue-uds-performance-mobile-troubleshooting`.
...
Related skills: `ue-uds-simulation`, `ue-uds-setup-and-modes`, `ue-uds-time`, `ue-uds-lighting-and-shadows`, `ue-uds-modifiers-configs-state`.
