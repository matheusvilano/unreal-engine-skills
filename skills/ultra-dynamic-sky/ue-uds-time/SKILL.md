...
UDS has a single canonical time variable, three categories of runtime control (set / animate / transition), four getter functions, and a complete event-dispatcher system for game logic that needs to react to time. For real-world astronomical positioning (lat/long/date), see `ue-uds-simulation`.
...
When `Simulate Real Sun` is enabled (see `ue-uds-simulation`), Day Length / Night Length are ignored — speed comes from **Simulation Speed** in the Simulation category (`1` = roughly real-time).
...
Good for materials that switch (e.g. emissive on at night) without involving a blueprint. For *light components* that turn on at night (not just materials), use the **Light Day/Night Toggle** component — see `ue-uds-lighting-and-shadows`.
...
- **`Time of Day` won't change at runtime** — the sun is **static or stationary mobility**. Time-of-day cannot change at runtime with non-movable sun. See `ue-uds-lighting-and-shadows`.
- **Keyframes in Sequencer have no effect** — `Animate Time of Day` is still on and overriding. Disable it.
- **Sunrise/Sunset firing at the wrong time** — `Simulate Real Sun` is on, which uses real horizon crossing rather than `Dusk Time` / `Dawn Time`. (See `ue-uds-simulation`.) Or there's a Sunrise/Sunset Event Offset configured.
...
Related skills: `ue-uds-simulation`, `ue-uds-sun-moon-stars`, `ue-uds-lighting-and-shadows`, `ue-uds-cinematics-rendering`, `ue-udw-setup-and-state`.
