...
For setting date from code, `Set Date and Time` and `Get Date Time` handle Date Time structs — see `ue-uds-time`.
...
`Time of Day` and the date variables remain settable directly per the standard runtime time-control functions in `ue-uds-time`; the simulation interprets them according to the configured location/DST.
...
Related skills: `ue-uds-time`, `ue-uds-sun-moon-stars`, `ue-uds-setup-and-modes`.
