...
| Modify UDS without losing on update | Child blueprints (see `ue-uds-modifiers-configs-state`) |
...
Movable is the default, but **static / stationary is always cheaper** if it fits the project. See `ue-uds-lighting-and-shadows` for setup.
...
Work in a **child class** rather than editing UDS directly. See `ue-uds-modifiers-configs-state` for the full pattern.
...
`Time of Day` and `Date` replicate via `Replicated Time of Day` periodic syncs. Clients tick time locally between updates so the wire traffic is minimal. Weather state in UDW replicates similarly via state-source asset references rather than direct state values (see `ue-udw-setup-and-state` replication notes).
...
Related skills: `ue-uds-setup-and-modes` (Sky / Color / Project / Feature Mode selection — the biggest perf decisions), `ue-uds-clouds` (full cloud settings; cloud rendering troubleshooting), `ue-uds-lighting-and-shadows` (Sky Light Mode tradeoffs; static lighting; exposure), `ue-uds-fog-and-atmosphere` (Volumetric Fog cost; hard fog lines; sky atmosphere position), `ue-uds-modifiers-configs-state` (child blueprints for modifying UDS), `ue-udw-spatial-weather` (child class pattern for Weather Override Volume).
