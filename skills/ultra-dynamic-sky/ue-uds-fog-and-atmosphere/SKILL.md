...
`Render Height Fog In Path Tracer Using Post Process` (in Fog Color category, on by default). Lets the path tracer approximate distant height fog via post process — the path tracer otherwise lacks native height fog support. See `ue-uds-cinematics-rendering`.
...
| Weather Mask brush masking | Mask volumetric fog where Weather Mask Brush / Projection Box actors are placed. See `ue-udw-material-and-screen-effects`. |
...
| Underwater extinction + albedo | Applied below the water level set in **Water Level** category. See `ue-uds-modifiers-configs-state`. |
...
If a runtime change has no effect, look for the matching `Static Properties - <Category>` function and call it. Last-resort full refresh: `Hard Reset Cache`. See `ue-uds-performance-mobile-troubleshooting`.
...
Related skills: `ue-uds-setup-and-modes`, `ue-uds-clouds`, `ue-uds-modifiers-configs-state` (water level), `ue-uds-cinematics-rendering`, `ue-udw-particles-lightning-wind-sounds` (UDW Dust flows into UDS Dust Amount), `ue-udw-material-and-screen-effects` (Weather Mask masking applies to Global Volumetric Material).
