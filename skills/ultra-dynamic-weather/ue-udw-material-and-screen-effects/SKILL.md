...
| Mode | Source |
| --- | --- |
| Default | Calculated temperature value (see `ue-udw-random-seasons-temperature`) |
...
If the actor has an **Actor Weather Status** component, the actor's local status drives the drip amount instead of global weather state. See `ue-udw-setup-and-state`.
...
The **Use UDS Water Level** option in UDW's **Water Level** category enables UDS water level for several UDW features. Actual water level config is on UDS — see `ue-uds-modifiers-configs-state`.
...
- **Screen Frost appearing indoors** — Player Occlusion isn't detecting the interior. See `ue-uds-modifiers-configs-state`.
...
Related skills: `ue-udw-setup-and-state` (weather/material state drives everything; Actor Weather Status for per-actor effects), `ue-udw-random-seasons-temperature` (temperature drives Freezing Breath, Heat Distortion, icicles), `ue-udw-spatial-weather` (Weather Mask system masks material effects), `ue-udw-particles-lightning-wind-sounds` (Foliage Wind references UDW wind state), `ue-uds-modifiers-configs-state` (UDS Water Level config; Player Occlusion drives Screen Frost interior masking), `ue-uds-fog-and-atmosphere` (Global Volumetric Material is the volumetric counterpart of Post Process Wind Fog).
