---
name: ue-udw-particles-lightning-wind-sounds
...
| **Player Occlusion masking** *(automatic)* | No shake when player is in an interior. Controlled by Player Occlusion on UDS — see `ue-uds-modifiers-configs-state`. |
...
The actual occlusion calculation lives in **Player Occlusion** on UDS — see `ue-uds-modifiers-configs-state`.
...
Related skills: `ue-udw-setup-and-state`, `ue-udw-spatial-weather` (WOV wind override; UDS Occlusion Volume), `ue-uds-modifiers-configs-state` (Player Occlusion calculates occlusion; AmbientSound — Time and Weather Controlled is the simpler sound option), `ue-udw-material-and-screen-effects` (material wetness/snow accumulates from particle landing context).
