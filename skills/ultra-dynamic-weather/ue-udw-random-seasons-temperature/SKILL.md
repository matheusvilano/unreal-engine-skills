---
name: ue-udw-random-seasons-temperature
...
`Change Weather` (see `ue-udw-setup-and-state`) switches to a static preset. To go back:
...
| --- | --- |
| Derived from date | Uses UDS's date (Simulation category, increments per day). See `ue-uds-simulation`. |
...
5. If sampleLocation != Global:
   ...
   - Apply Interior Temperature based on Player Occlusion of the sample point
     (Player Occlusion is on UDS — see ue-uds-modifiers-configs-state)
```

### Temperature and Weather Override Volumes

Weather Override Volumes apply their own temperature ranges while the player is inside. Configurable per-WOV or disablable. See `ue-udw-spatial-weather`.
...
- **`GetCurrentTemperature(local)` reads same as Global** — sample location is set to Global, or there are no Temperature Volumes / Player Occlusion isn't computing occlusion at that location. See `ue-uds-modifiers-configs-state`.
- **Climate preset overwrote a custom probability map** — applying a climate preset is destructive to the maps. Save a config asset first (see `ue-uds-modifiers-configs-state`) before experimenting.
...
Related skills: `ue-udw-setup-and-state`, `ue-uds-simulation` (date drives season when Season Mode set to derive), `ue-udw-spatial-weather` (WOVs override temperature ranges locally), `ue-uds-modifiers-configs-state` (Player Occlusion drives Interior Temperature), `ue-udw-material-and-screen-effects` (Freezing Breath uses calculated temperature).
